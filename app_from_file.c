...
int main()
{
	...
	int ok = SDL_CreateWindowAndRenderer("resource from file example",720, 720, 0, &window, &renderer);
	if (ok < 0)
	{
	...
	unsigned char *img_bytes = stbi_load("waving-person.png", &img_x, &img_y, &img_channels, 4);
	if(img_bytes == NULL)
	{
	...
	while(1)
	{
...

		{
			...
			// WAIT 16 milliseconds for 60 FPS loop
			SDL_Delay(16);
		}
	}
}